...
`Adam` should appear before `Zed` in the list as shown in Figure
* If `Zed` has `important` tag then +
`Zed` should appear before `Adam` in the list as shown in Figure
* If another person called `Aloy` with `important` tag is added then +
`Aloy` will appear before `Zed` and is before `Adam` as shown in Figure
// end::sort[]
